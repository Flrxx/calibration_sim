...
def calculate_optimal_position(model: hayati_model.HayatiModel, i_target: int, angles_start:Union[list, np.ndarray],
    ...
    def objective(angles):
        ...
        return -(output[i_target]**2  - 3 * np.sum(output[0:i_target]**2) - 5 * np.sum(output[i_target + 1:]**2))  #- max(np.concatenate([output[0:i_target], output[i_target + 1: border]]))**2
    ...
    def wire_limits(angles):    
        ...
        angle_cons_wire = np.array([alpha_wire, model.angle_limit_wire - alpha_wire]) # so angle would fit
        ...
        alpha = acos(scalar)
        angle_cons = np.array([alpha, model.angle_limit_wire - alpha]) # so angle would fit
