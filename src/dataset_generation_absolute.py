...
def generate_dataset(model: hayati_model.HayatiModel):
    ...
    write_dataset(generate_random_dataset(model), model.dataset_file, FIELDNAMES_OPTIONS["random"])
...
def generate_random_dataset(model, disable_limits=False):
    dataset = np.zeros((model.general_samples_number, len(FIELDNAMES_OPTIONS["random"])))
